...
--          nodeMethod = on_start
class NodeMethod node (name :: Symbol) (args :: [ * ]) (ret :: *)
...
data PropertyAttributes
    = PropertyAttributes { propertySetType :: !MethodRpcMode, propertyType
          :: !VariantType, propertyHint :: !PropertyHint, propertyHintString
          :: !Text, propertyUsage :: !PropertyUsageFlags, propertyDefaultValue
          :: !(Variant 'GodotTy) }

data ClassProperty cls
    = ClassProperty { propertyName :: Text, propertyAttrs
          :: PropertyAttributes, propertySetter :: Object -> cls -> GodotVariant
          -> IO (), propertyGetter :: Object -> cls -> IO GodotVariant }
...
class ( Typeable ty, AsVariant ty )
    => NodeProperty node (name :: Symbol) ty (ro :: Bool)
    | node name -> ty, node name -> ro where
...
deriveBase ty = do
    ...
    case r of
        FamilyI _ instDecs -> do
            liftM nub $ concatQ
                (\( child, parent ) -> do
                     liftM2 (++) [d| instance $( pure appliedTy
                                              ) :< $( pure appliedTy ) where
                                         upcast = id
                                 |] (parentInstances instDecs child child))
                $ relevantTySynEqns (map (\(TySynInstD eqn) -> eqn) instDecs)
                (ConT ty)
...
relevantTySynEqns (TySynEqn _ (AppT _ child) parent : ts) ty
  ...
  where
    ...
    isRelevant t ty = error $ "Unhandled type family member " ++ show t
        ++ " when making parent instances for type " ++ show ty
relevantTySynEqns x ty = error $ "Unhandled type member " ++ show x
    ++ " when deriving parent instances for type " ++ show ty
...
parentInstances instDecs superType targetType = concat
    <$> (mapM
         (\( child, parent ) -> if parent == targetType then pure []
          else (liftM2 (++)
                [d| instance $( pure parent ) :< $( pure targetType ) where
                        upcast = upcast . super
                |] (if parent == superType then pure []
                    else (parentInstances instDecs parent targetType))))
         $ filter (\( child, parent ) -> compatible superType child) $ map
         (\(TySynInstD (TySynEqn _ (AppT _ child) parent)) -> ( child, parent ))
         instDecs)
